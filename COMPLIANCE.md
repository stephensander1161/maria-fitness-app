...
| Budgets, one-day top-ups and the owner role are written from an owner-gated **route**, never a tool — a prompt that could grant one could buy itself an unlimited day. Same `lib/budget.ts` arithmetic as the command line, so the screen cannot be more permissive; every change audited | `app/api/admin/account/route.ts`, `lib/budget.ts` |
| Removing the last owner is refused, and a role change requires the account's email retyped | `app/api/admin/account/route.ts` |
| A push still carries no content: the service worker asks the app what the notification is about over its own session, so the push service sees an empty envelope | `public/sw.js`, `app/api/push/pending/route.ts`, `lib/push.ts` |
